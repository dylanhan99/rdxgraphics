...
#define _RX_XX(Klass) allSame &= objekt.GetVBData<Klass>().size() == sz;
	if constexpr (std::is_same_v<T, VertexBasic>)
	{
		...
		// Use precalculated BVs if not empty (cus from assimp) else, just use our own vertices (cus generated by us)
		std::vector<glm::vec3> const& bvPositions = _RX_TUP(5);
		std::vector<glm::vec3> const& data = bvPositions.empty() ?
			objekt.GetVBData<VertexBasic::Position>() :
			bvPositions;

		Intersection::CalculateAABBBV(
			data,
			objekt.GetDefaultAABBBV().GetOffset(),
			objekt.GetDefaultAABBBV().GetHalfExtents()
		);
	}
	...
	asd =
		[&](aiNode* pNode)
		{
			...
			for (int i = 0; i < pNode->mNumMeshes; ++i)
			{
				...
				{
					aiAABB const& aabb = pMesh->mAABB;
					minmaxes.emplace_back(glm::vec3{ aabb.mMin.x,aabb.mMin.y,aabb.mMin.z });
					minmaxes.emplace_back(glm::vec3{ aabb.mMax.x,aabb.mMax.y,aabb.mMax.z });
				}

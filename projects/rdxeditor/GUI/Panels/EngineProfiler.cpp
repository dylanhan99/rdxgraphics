...
void EngineProfiler::UpdateImpl(float dt)
{
	...
	ImGui::BeginDisabled(true);
	{
		...
		{
			rdx::ServiceLayer::InstantEventService()->Publish(
				rdx::TemporaryProfilingEvent{ duration });
		}
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("Idk about this feature honestly, awkward for editor end.\nFeels more appropriate for timed capture, then output to file.");
		ImGui::SameLine();
		{
			ImGui::DragFloat("##Profile Duration", &duration, 1.f, 0.f, FLT_MAX, "%.1f");
		}
	}
	ImGui::EndDisabled();

	{
		static bool isAlwaysProfiling{};
		isAlwaysProfiling = rdx::ServiceLayer::PerformanceProfilingService()->IsAlwaysProfiling();
		if (ImGui::Checkbox("IsAlwaysProfiling", &isAlwaysProfiling))
		{
			rdx::ServiceLayer::InstantEventService()->Publish(
				rdx::TogglePerformanceProfileEvent{ isAlwaysProfiling });
		}
	}
}

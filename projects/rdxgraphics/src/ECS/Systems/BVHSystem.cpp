...
{
	auto cacheSort =
		[](std::pair<float, BVHNode> const& lhs, std::pair<float, BVHNode> const& rhs)
		{
			return lhs.first < rhs.first;
		};

	// It's the internal vector that's sorted
	std::map<entt::entity, std::vector<std::pair<float, BVHNode>>> sortedCache{};
